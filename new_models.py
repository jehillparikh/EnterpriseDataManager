...
class Fund(db.Model):
    ...
    # Relationships
    factsheet = db.relationship("FundFactSheet", backref="fund", uselist=False, cascade="all, delete-orphan")
    returns = db.relationship("FundReturns", backref="fund", uselist=False, cascade="all, delete-orphan")
    portfolio_holdings = db.relationship("PortfolioHolding", backref="fund", cascade="all, delete-orphan")
    nav_history = db.relationship("NavHistory", backref="fund", cascade="all, delete-orphan")
...
class FundFactSheet(db.Model):
    ...
    isin = db.Column(db.String(12), db.ForeignKey('fund.isin'), primary_key=True)
...
class PortfolioHolding(db.Model):
    ...
    isin = db.Column(db.String(12), db.ForeignKey('fund.isin'), nullable=False)
